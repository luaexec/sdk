...
template <class t, class a = int> class utlmemory_t {
public:
	...
	[[nodiscard]] inline int num_allocated() const {
	...
	/*void grow( int num ) {
		...
		int new_allocation_count = calc_new_allocation_count( allocation_count, grow_size, allocation_requested, sizeof( t ) );
...
inline t &utlmemory_t<t, a>::operator[]( a i ) {
	assert( is_idx_valid( i ) );
	return memory[ i ];
}

template< class t, class a >
inline const t &utlmemory_t<t, a>::operator[]( a i ) const {
	assert( is_idx_valid( i ) );
	return memory[ i ];
}

template< class t, class a = utlmemory_t<t> >
class utl_vector {
...
public:
	...
	[[nodiscard]] inline bool is_valid_index( int i ) const {
		return ( i >= 0 ) && ( i < size );
	}
